"""Petals P121: three rotational hops out and three inverse hops home."""
...
def main() -> None:
    for rotation in ROTATIONS:
        for point in SHELL:
            trace = six_hops(point, rotation)
            assert len(trace) == 7
            assert trace[0] == trace[6]
            assert all(state in SHELL for state in trace)
    print("P121 PASS: 3 hops out plus 3 hops back returns every shell node home")
